...
```bash
claude mcp add stellarsight -- node /absolute/path/to/repo/apps/agent/src/mcp-server.mjs
```
...
// output
{
  ...
  "items": [{
    ...
    "url": "...", "serviceName": "stellarsight-fx", "description": "...", "tags": ["fx"],
...
// output
{
  ...
  "howToCall": { "tool": "stellarsight_pay", "url": "...", "method": "GET", "params": {...},
                 "methodHint": "Pass params as the query string (stellarsight_pay defaults to GET).",
                 "note": "..." },
  "source": "http://localhost:4022"
}
```
...
|---|---|---|
| `STELLARSIGHT_CONFIG_MISSING` | `PAYER_SECRET` absent or not a valid `S...` seed | run `scripts/setup-testnet.mjs`, or set it in the MCP `env` block |
| `STELLARSIGHT_BAD_REQUEST` | malformed argument (empty url/query/id, non-integer `maxPrice`) | fix the argument named in the reason |
| `STELLARSIGHT_INDEX_UNREACHABLE` | the discovery index did not answer | start the stack (`npm run dev:all`) or set `INDEX_URL` |
| `STELLARSIGHT_INDEX_ERROR` | index answered non-2xx or non-JSON | check the index logs; the HTTP status is in the reason |
| `STELLARSIGHT_NO_RESULTS` | nothing matched the query or the filters | broaden the query, or raise `maxPrice` |
| `STELLARSIGHT_NOT_FOUND` | no resource with that id is registered | get a valid id from `stellarsight_search` / `stellarsight_browse` |
| `STELLARSIGHT_RESOURCE_UNREACHABLE` | the seller endpoint refused the connection | the seller is down, or the URL is wrong |
| `STELLARSIGHT_402_MALFORMED` | 402 with no decodable `PAYMENT-REQUIRED` header and no `accepts` body | the seller is not speaking x402 v2 |
| `STELLARSIGHT_UNSUPPORTED_NETWORK` | resource wants a network/scheme this agent is not configured for | testnet only; check `STELLAR_NETWORK` |
| `STELLARSIGHT_PRICE_EXCEEDS_BUDGET` | quoted price is above the caller's `maxPrice` | raise `maxPrice`, or pick a cheaper resource |
| `STELLARSIGHT_SIGN_FAILED` | the Soroban auth entry could not be built or signed | check RPC reachability and the payer account |
| `STELLARSIGHT_INSUFFICIENT_BALANCE` | payer lacks the asset, or has no trustline | fund the payer / add the trustline |
| `STELLARSIGHT_REPLAY_REJECTED` | the payment authorization had already been consumed | sign a fresh payment; never reuse a header |
| `STELLARSIGHT_AUTH_EXPIRED` | the auth entry's ledger bounds had passed | retry — signing is cheap |
| `STELLARSIGHT_SETTLE_FAILED` | facilitator returned `success: false` for another reason | the facilitator's `errorReason` is quoted in the reason |
| `STELLARSIGHT_UPSTREAM_ERROR` | unexpected non-402 HTTP error, or an internal throw | read the reason; the HTTP status is included |
| `STELLARSIGHT_TIMEOUT` | the resource or index exceeded the timeout | raise `timeoutMs` |
...
| `PAYER_SECRET` | — | the agent's wallet; **required for `stellarsight_pay`** |
...
- 402 challenge arrives in the **`PAYMENT-REQUIRED`** response header (base64 JSON). STELLARSIGHT
  also accepts the same object in the JSON body, which is what several v2 servers emit.
- The signed payload goes out as **`PAYMENT-SIGNATURE`** (x402 v2); STELLARSIGHT mirrors it onto
