...
Palette: accent `#F5400E` · ink `#0B0C0E` · surface `#121215` · cream `#F2EDE3`

| File | What it is | Intended use |
| --- | --- | --- |
| `paymap-mark.svg` | 512×512 — the mark: accent pin, ink core, white four-pointed star. Transparent ground. | Standalone mark at 24–128px (README header, footer). The nav and search chrome use an inline-SVG mirror of this file (`src/components/Marks.tsx`), so the UI never depends on it resolving. |
| `paymap-mark.png` | Transparent raster of the mark. | Contexts that cannot take SVG (embeds, chat previews). |
