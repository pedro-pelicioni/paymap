...
| Silent spec drift | stock-client conformance vs the live stack; 46 API checks + suite nightly in CI | Caught a real bug already |
...
the conformance cadence is scheduled to continue past the grant window: CI runs the 46
...
| Integrity is real | `npm test` → 200 tests, 66 adversarial | 30s |
