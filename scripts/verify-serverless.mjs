...
async function main() {
  console.log('\nSTARSIGHT serverless verification\n');
  ...
  await check("STARSIGHT's additive fields survive the spec projection", () => {
  ...
  await check('`items` is still emitted as a deprecated alias of the same array', () => {
    // One release of grace for consumers written against the old STARSIGHT envelope.
  ...
  await check('store configured but no write token: still refused, with the reason', async () => {
    ...
    try {
      ...
      eq(res.statusCode, 503, 'status');
      assert(/STARSIGHT_WRITE_TOKEN/.test(res.json.reason), `reason should name the token, got: ${res.json.reason}`);
    } finally {
      restore();
    }
  });
  ...
  await check('store + token, wrong bearer: 401', async () => {
    ...
    try {
      ...
      await resourcesHandler(
        ...
        res,
        { ...KV_ENV, STARSIGHT_WRITE_TOKEN: 's3cret' },
      );
  ...
  await check('store + token + bearer: the record is validated, persisted and then served', async () => {
    ...
    const env = { ...KV_ENV, STARSIGHT_WRITE_TOKEN: 's3cret', STARSIGHT_KV_TTL_MS: '0' };
    try {
      const write = mockRes();
      await resourcesHandler(
  ...
  try {
    ...
    await check('a stock consumer can go from one search hit to a payable offer', async () => {
      ...
      // and the recipient off `accepts[0]` with no STARSIGHT-specific knowledge at all.
    ...
    await check('STARSIGHT extras ride along without displacing a spec field', async () => {
