...
function writeEnv(vals) {
  const body = `# PAYMAP — generated by scripts/setup-testnet.mjs
...
function writeTxDoc() {
  ...
  const rows = txs
    ...
    .join("\n");
  const header = `# PAYMAP — testnet transactions
...
async function main() {
  log("\nPAYMAP testnet setup — no faucets, no captchas, no API keys.\n");
  ...
  await submitClassic("Issuer setOptions (home domain)", issuer.kp, (b) =>
    b.addOperation(Operation.setOptions({ homeDomain: "paymap.local" })),
  );
