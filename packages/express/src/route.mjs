...
function warnAboutUndocumentedPathParams(declaration, path, method, logger) {
  ...
  logger.warn(
    `[paymap] ${method} ${path} takes ${missing.length === 1 ? "a path parameter" : "path parameters"} ` +
...
function normalizeTags(value, logger) {
  ...
  if (tags.length > 5) {
    logger.warn(
      `[paymap] this route declares ${tags.length} tags; the bazaar keeps at most 5, so ${tags.slice(5).join(", ")} will be dropped by the index.`,
    );
  }
  return tags;
}
...
function normalizeServiceName(value, logger) {
  ...
  if (name && name.length > 32) {
    logger.warn(
      `[paymap] serviceName ${JSON.stringify(name)} is ${name.length} characters; the bazaar drops names longer than 32, so this route will be listed without one.`,
    );
  }
