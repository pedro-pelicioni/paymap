 ...
 * wire format, and it is where STARSIGHT's internal catalog record (CONTRACT.md) is
 ...
 *
 * STARSIGHT stores that payload flattened onto the record (`input`, `output`,
 ...
 *
 * Everything STARSIGHT-native is kept as ADDITIVE fields the spec does not define and a
 ...
 * @returns {object}    a spec `DiscoveryResource` plus STARSIGHT's additive fields
 */
export function toDiscoveryResource(rec) {
  ...
  const out = {
    ...
    /* ── additive, STARSIGHT-native: not in the spec, ignored by a spec client ── */
    id: rec.id ?? url,
    network,
    scheme,
    payTo,
 ...
 * release, for consumers written against the old STARSIGHT envelope. It is not spec and
 * will be removed.
 *
