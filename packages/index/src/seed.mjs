/**
 * packages/index/src/seed.mjs — STARSIGHT demo catalog.
 *
...
const ICON = (name) => `https://cdn.starsight.build/icons/${name}.svg`;
...
export const SEED_RECORDS = [
  ...
  },
  {
    ...
    ...base,
    resource: {
      url: 'https://mcp.starsight.example/mcp',
      serviceName: 'STARSIGHT Catalog MCP',
      description:
        'Search the STARSIGHT bazaar catalog for paid x402 resources on Stellar by natural-language description, returning ranked results with pricing.',
      tags: ['mcp', 'discovery', 'search', 'x402', 'tools'],
      iconUrl: ICON('starsight'),
    },
    type: 'mcp',
    payTo: PAY_B,
...
if (process.argv[1] && import.meta.url === new URL(`file://${process.argv[1]}`).href) {
  ...
  console.log(`[starsight] wrote ${r.count} records to ${r.path}`);
}
